/**
 * Build a coarse raster "heatmap" of DVF house €/m² (median per grid cell) around Lyon.
 *
 * Source: DVF géolocalisées — fichiers département 69 par année
...
const LYON_ZONES = path.join(BASE, 'map-data', 'lyon-zones.json');
const OUT = path.join(BASE, 'public', 'lyon-dvf-heatmap.json');
...
const DEFAULTS = {
  ...
  eurM2Min: 1000,
  eurM2Max: 10000,
  maxPerCell: 4000 // cap memory; enough for medians
};
    ...
    stream.on('data', (chunk) => {
      ...
      while (true) {
        ...
        if (!header) {
          ...
          col = {
            date_mutation: pos.date_mutation,
        ...
        // Même ordre que lyon-iso-grid : y=0 = Nord (maxLat), y=ny-1 = Sud (minLat)
...
async function main() {
  const z = readJson(LYON_ZONES);
  const o = (z && z.origin) ? z.origin : null;
  if (!o || !Number.isFinite(o.lat) || !Number.isFinite(o.lng)) {
    throw new Error('lyon-zones.json: missing origin lat/lng');
  }
