/**
 * Build an OSRM driving-duration grid around Lyon origin for isochrone rendering.
 *
 * Output:
 * - map-data/lyon-iso-grid-osrm.json (source)
 * - public/lyon-iso-grid.json (served)
 *
 * Approach:
...
const IN_ZONES = path.join(BASE_DIR, 'map-data', 'lyon-zones.json');
const OUT_MAPDATA = path.join(BASE_DIR, 'map-data', 'lyon-iso-grid-osrm.json');
const OUT_PUBLIC = path.join(BASE_DIR, 'public', 'lyon-iso-grid.json');

const OSRM_BASE = process.env.OSRM_BASE || 'https://router.project-osrm.org';
const USER_AGENT = 'usa-interactive-map/2.0 (Lyon OSRM isochrone grid; local dev)';
...
async function main() {
  ...
  if (!origin || !Number.isFinite(origin.lat) || !Number.isFinite(origin.lng)) {
    throw new Error('Missing zones.origin lat/lng in map-data/lyon-zones.json');
  }
